package com.mycompany.td1.dasi.dao;
...
public class JpaUtil {
     ...
     */
    public static final String PERSISTENCE_UNIT_NAME = "com.mycompany_TD1-DASI_jar_1.0-SNAPSHOTPU";
    /**
    ...
    public static Object trouver(Class<?> aClass, Long id) {
        EntityManager em = threadLocalEntityManager.get();
        if(aClass == Client.class) {
            return em.find(aClass, id);
        }
        return null;
    }
    
    public static List<Object> trouverListe(Class<?> aClass) {
        EntityManager em = threadLocalEntityManager.get();
        if(aClass == Client.class) {
            String queryStr = "SELECT c FROM Client c";
            TypedQuery query = em.createQuery(queryStr, Client.class);
